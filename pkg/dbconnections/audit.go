...
func initAuditCollections(db *mongo.Database) {
	...
	_, _ = AuditLogsCollection.Indexes().CreateOne(ctx, mongo.IndexModel{
		Keys: bson.D{{Key: "engagement_id", Value: 1}, {Key: "time", Value: -1}},
	})
}

// InsertAuditLog appends an audit event (best-effort; errors are logged by callers).
// engagementID is a Mongo ObjectId hex for the engagement, or empty for global events (e.g. user creation, master exports).
func InsertAuditLog(ctx context.Context, actor, action string, details bson.M, engagementID string) error {
	e := AuditLogEntry{
		Time:    time.Now().UTC(),
		Actor:   actor,
		Action:  action,
	...
	if id := strings.TrimSpace(engagementID); id != "" {
		e.EngagementID = id
	}
...
// ListAuditLogsByEngagement returns newest entries for one engagement (hex id).
func ListAuditLogsByEngagement(ctx context.Context, engagementIDHex string, limit int64) ([]AuditLogEntry, error) {
	if limit < 1 || limit > 10000 {
		limit = 500
	}
	engagementIDHex = strings.TrimSpace(engagementIDHex)
	if engagementIDHex == "" {
		return nil, nil
	}
	cur, err := AuditLogsCollection.Find(ctx, bson.M{"engagement_id": engagementIDHex},
		options.Find().SetSort(bson.D{{Key: "time", Value: -1}}).SetLimit(limit))
	if err != nil {
		return nil, err
	}
	defer cur.Close(ctx)
...
// ListAuditLogsForEngagementExport returns up to max entries for one engagement (newest first).
func ListAuditLogsForEngagementExport(ctx context.Context, engagementIDHex string, max int64) ([]AuditLogEntry, error) {
	if max < 1 {
		max = 50000
	}
	if max > 100000 {
	...
	cur, err := AuditLogsCollection.Find(ctx, bson.M{"engagement_id": engagementIDHex},
		options.Find().SetSort(bson.D{{Key: "time", Value: -1}}).SetLimit(max))
	if err != nil {
		return nil, err
	}
	defer cur.Close(ctx)
